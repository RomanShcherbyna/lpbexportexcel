...
<div class="box">
    ...
    <div class="section">
        ...
        <p class="muted">Hard rules (SKU/name/price/duplicates). These are separate from AI warnings.</p>
        @php $dc = $preview['deterministic']['counts'] ?? []; @endphp
        <table>
            <tr><th>Duplicate SKU</th><td>{{ $dc['duplicate_sku_count'] ?? 0 }}</td></tr>
            <tr><th>Missing SKU</th><td>{{ $dc['missing_sku_count'] ?? 0 }}</td></tr>
            <tr><th>Missing name</th><td>{{ $dc['missing_name_count'] ?? 0 }}</td></tr>
            <tr><th>Missing price</th><td>{{ $dc['missing_price_count'] ?? 0 }}</td></tr>
            <tr><th>Completely empty rows</th><td>{{ $dc['empty_row_count'] ?? 0 }}</td></tr>
        </table>
    </div>
